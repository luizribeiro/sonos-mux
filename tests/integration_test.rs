...
fn test_muxd_starts_and_stops() {
    ...
    let mut child = Command::new("cargo")
        .args(&["run", "--bin", "muxd", "--", "--config", "examples/config.toml"])
        .stdout(Stdio::null())
        .stderr(Stdio::null())
        .spawn()
    ...
    #[cfg(unix)]
    {
        assert!(status.code().is_none(), "Expected muxd to be killed by signal");
    }
    
    // On Windows, we can't check the specific signal, so we just make sure it exited
    #[cfg(windows)]
    {
        assert!(!status.success(), "Expected muxd to exit with non-zero status");
    }
}